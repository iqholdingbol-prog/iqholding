...
'@ | Out-Null
    # Diagnóstico efímero del primer resultado que satisface el EXISTS final de
    # tipos. No modifica el DDL versionado, no altera ACL y sólo expone
    # metadatos técnicos de pg_type en el log efímero de CI.
    $phase1TypeUsageDeclarePath = $null
    $phase1TypeUsageDiagnosticPath = $null
    try {
        $phase1TypeUsageDeclarePath = New-InjectedSql `
            -SourcePath $SchemaPath `
            -Pattern '(?m)^DO \$verificar_postura_seguridad\$\r?\nBEGIN$' `
            -Replacement ('DO $verificar_postura_seguridad$' + [Environment]::NewLine + 'DECLARE' + [Environment]::NewLine + '    v_type_usage_diagnostic record;' + [Environment]::NewLine + 'BEGIN') `
            -Name 'phase1_type_usage_diagnostic_declare.sql'

        $phase1TypeUsageDiagnosticSql = @"
    SELECT
           role_iqg.role_name AS role_name,
           schema_iqg.nspname AS schema_name,
           type_iqg.oid::text AS type_oid,
           type_iqg.typname,
           type_iqg.typtype,
           type_iqg.typrelid::text AS typrelid,
           type_iqg.typowner::regrole::text AS typowner,
           COALESCE(type_iqg.typacl::text, '<null>') AS typacl,
           array_to_string(
               COALESCE(
                   type_iqg.typacl,
           ...
           CASE
               WHEN type_iqg.typtype = 'c' AND type_iqg.typrelid <> 0 THEN 'table_row_type'
               WHEN type_iqg.typcategory = 'A' AND type_iqg.typelem <> 0 THEN 'array_type'
               WHEN type_iqg.typtype = 'd' THEN 'domain'
               WHEN type_iqg.typtype = 'e' THEN 'enum'
               WHEN type_iqg.typtype = 'c' THEN 'independent_composite'
               WHEN type_iqg.typtype = 'b' THEN 'base_type'
               ELSE 'other'
           END AS classification,
           CASE
               WHEN type_iqg.typowner = to_regrole(role_iqg.role_name::text) THEN 'owner'
               WHEN EXISTS (
                   SELECT 1
               ...
               WHEN EXISTS (
                   SELECT 1
                    ...
                    WHERE acl.grantee = to_regrole(role_iqg.role_name::text)
                      AND acl.privilege_type = 'USAGE'
               ) THEN 'DIRECT_ROLE_ACL'
               WHEN type_iqg.typacl IS NULL THEN 'IMPLICIT_DEFAULT_OR_SPECIAL'
               ELSE 'OTHER_ACL_OR_MEMBERSHIP'
           END AS privilege_source
      INTO v_type_usage_diagnostic
     ...
     ORDER BY role_iqg.role_name, schema_iqg.nspname, type_iqg.oid
     LIMIT 1;
    ...
    IF FOUND THEN
        RAISE EXCEPTION USING
            ERRCODE = 'P0001',
            MESSAGE = format(
                'PHASE1_TYPE_USAGE_DIAGNOSTIC role=%s schema=%s type_oid=%s typname=%s typtype=%s typrelid=%s typowner=%s typacl=%s effective_typacl=%s classification=%s privilege_source=%s has_type_privilege_result=true',
                v_type_usage_diagnostic.role_name,
                v_type_usage_diagnostic.schema_name,
                v_type_usage_diagnostic.type_oid,
                v_type_usage_diagnostic.typname,
                v_type_usage_diagnostic.typtype,
                v_type_usage_diagnostic.typrelid,
                v_type_usage_diagnostic.typowner,
                v_type_usage_diagnostic.typacl,
                v_type_usage_diagnostic.effective_typacl,
                v_type_usage_diagnostic.classification,
                v_type_usage_diagnostic.privilege_source
            );
    END IF;
"@
        $phase1TypeUsageDiagnosticPath = New-InjectedSql `
            -SourcePath $phase1TypeUsageDeclarePath `
            -Pattern '(?ms)^\s*IF EXISTS \(\s*SELECT 1\s+FROM pg_type AS type_iqg\s+JOIN pg_namespace AS schema_iqg ON schema_iqg\.oid = type_iqg\.typnamespace\s+CROSS JOIN \(VALUES\s+\(''iqg_app''::name\),\s+\(''iqg_gateway''::name\),\s+\(''iqg_bootstrap_invoker''::name\)\s+\) AS role_iqg\(role_name\)' `
            -Replacement ($phase1TypeUsageDiagnosticSql + [Environment]::NewLine + '$0') `
            -Name 'phase1_type_usage_diagnostic.sql'

        Invoke-PsqlFile -Case 'PHASE1_empty_wrong_owner_normalized' -Path $phase1TypeUsageDiagnosticPath -Database 'iqg_phase1_wrong_owner_probe' -User 'postgres' | Out-Null
    }
    finally {
        if ($phase1TypeUsageDiagnosticPath) {
            Remove-Item -LiteralPath $phase1TypeUsageDiagnosticPath -Force -ErrorAction SilentlyContinue
        }
        if ($phase1TypeUsageDeclarePath) {
            Remove-Item -LiteralPath $phase1TypeUsageDeclarePath -Force -ErrorAction SilentlyContinue
        }
    }
